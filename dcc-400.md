...
### FIND MAC Address   
```# ============================================================
#  DCC-400  -  FIND MY MAC ADDRESS
#  Run this on the ARM (receiver) ESP32 FIRST.
#  It prints the board's MAC address. Copy that whole b'...'
#  line into the PEER setting inside wrist_sender.py
# ============================================================
...
print("This board's MAC address is:")
print(sta.config('mac'))
print()
print("Copy the b'...' line above into PEER in wrist_sender.py")```


### Wrist Sender  

# ============================================================
#  DCC-400  -  WRIST SENDER
#  ESP32 + MPU-6050, talking over ESP-NOW
#
#  This is the part you WEAR. It feels how your wrist is
#  tilted and beams the angles to the arm.
#
#  >>> Run find_mac.py on the ARM board first, then paste
#      its MAC address into PEER below. <<<
# ============================================================

from machine import Pin, I2C
import network, espnow, struct, math, time

# ---- STEP 1: paste your ARM board's MAC address here ----
PEER = b'\x24\x6f\x28\x00\x00\x00'      # <-- CHANGE THIS

# ---- STEP 2: turn on the radio (ESP-NOW needs WiFi on) ----
sta = network.WLAN(network.STA_IF)
sta.active(True)

e = espnow.ESPNow()
e.active(True)
e.add_peer(PEER)                        # remember who we're talking to

# ---- STEP 3: wake up the MPU-6050 motion sensor ----
i2c = I2C(0, scl=Pin(22), sda=Pin(21))
MPU = 0x68
i2c.writeto_mem(MPU, 0x6B, b'\x00')     # wake it from sleep

def read_word(reg):
    # Read 2 bytes from the sensor and turn them into one
    # signed number (it can be negative).
...
def read_tilt():
    ax = read_word(0x3B)                # accelerometer X
    ay = read_word(0x3D)                # accelerometer Y
    az = read_word(0x3F)                # accelerometer Z
    # Turn those raw numbers into tilt angles, in degrees.
    pitch = math.degrees(math.atan2(ax, math.sqrt(ay*ay + az*az)))
    roll  = math.degrees(math.atan2(ay, math.sqrt(ax*ax + az*az)))
    return int(pitch), int(roll)

# ---- STEP 4: read the wrist and send, over and over ----
while True:
    pitch, roll = read_tilt()
    # Pack the two angles into a tiny 4-byte bundle (a "struct").
    # 'hh' means two whole numbers that can be negative.
    message = struct.pack('hh', pitch, roll)
    e.send(PEER, message)
    print("Sent  pitch:", pitch, "  roll:", roll)
    time.sleep(0.02)                    # about 50 times a second```
...
### Arm Receiver  
# ============================================================  
#  DCC-400  -  ARM RECEIVER
#  ESP32 + 4x SG90 servos, listening over ESP-NOW
#
#  This is the ARM on the desk. It listens for the wrist
#  angles and moves the joints to match your hand.
#
#  POWER REMINDER: the servos get their OWN 5V/6V power.
#  Do NOT power them from the ESP32. Just share a common GND.
# ============================================================

from machine import Pin, PWM
import network, espnow, struct

# ---- STEP 1: turn on the radio so we can listen ----
sta = network.WLAN(network.STA_IF)
sta.active(True)
...
# ---- STEP 2: set up the 4 servos on 4 pins ----
base     = PWM(Pin(13), freq=50)        # joint 1
shoulder = PWM(Pin(12), freq=50)        # joint 2
elbow    = PWM(Pin(14), freq=50)        # joint 3
gripper  = PWM(Pin(27), freq=50)        # joint 4

def set_angle(servo, angle):
    # Keep the angle safely between 0 and 180.
    if angle < 0:
        angle = 0
    if angle > 180:
        angle = 180
    # Turn the angle into the signal an SG90 servo understands.
    duty = int(1638 + (angle / 180) * (8192 - 1638))
    servo.duty_u16(duty)

def to_servo(angle):
    # Wrist tilt is about -90 to +90. Servos like 0 to 180.
    # Shift it across: -90 becomes 0, 0 becomes 90, +90 becomes 180.
    return angle + 90

# Park the arm in a comfy middle pose to start.
set_angle(base, 90)
set_angle(shoulder, 90)
set_angle(elbow, 90)
set_angle(gripper, 90)

# ---- STEP 3: listen forever and move the arm ----
while True:
    host, message = e.recv()            # wait for the next wrist update
    if message:
        pitch, roll = struct.unpack('hh', message)
        set_angle(shoulder, to_servo(pitch))   # tilt forward/back -> shoulder
        set_angle(base,     to_servo(roll))    # tilt left/right   -> base
        print("Got  pitch:", pitch, "  roll:", roll)```
