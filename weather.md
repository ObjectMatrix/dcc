...
<h1> Where in the World Are You? Can you find your spot using lines of Latitude and Longitude?</h1>
...
```
def get_location_in(pin_code):
    """Get location info (city, state, lat, long) from an Indian PIN code"""
    if not (pin_code.isdigit() and len(pin_code) == 6):
        return {"error": "That doesn't look like a valid PIN code."}

    try:
        zip_data = requests.get(
            f"http://api.zippopotam.us/in/{pin_code}", timeout=10
        )
        if zip_data.status_code == 404:
            return {"error": f"PIN code {pin_code} was not found."}
        place = zip_data.json()["places"][0]
