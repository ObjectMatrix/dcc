# Give Your Chip a Brain: ESP32 + Python Setup (macOS)

## Thonny + MicroPython on the ESP32-WROOM-32

This guide gets your **Mac** ready to program an **ESP32-WROOM-32** in Python. You will do **everything from inside the Thonny app using the mouse or keyboard**. No terminal, no typed commands, nothing to memorize.
...
1. Connect the ESP32-WROOM-32 to your Mac with a **data USB cable**.
2. Most WROOM boards use a **CP2102** or **CH340** USB-to-serial chip. Modern macOS usually detects these on their own. If your board is not detected later, install the matching driver:
   - CP2102: search "Silicon Labs CP210x driver"
   - CH340: search "WCH CH340 driver"

How the port will show up on macOS:

`/dev/cu.usbserial-XXXX` or `/dev/cu.SLAB_USBtoUART` or `/dev/cu.wchusbserial-XXXX`
...
This is the optional command-line way. Most students should use the Thonny GUI method in Part 3. On macOS the port is a `/dev/cu.*` path.

Erase it:
`esptool.py --chip esp32 --port /dev/cu.usbserial-0001 --baud 460800 erase_flash`

Burn it:
`esptool.py --chip esp32 --port /dev/cu.usbserial-0001 --baud 460800 write_flash -z 0x1000 ./ESP32_GENERIC-20260406-v1.28.0.bin`

> **If `esptool.py` is not found:** install it once with `pip3 install esptool`.

### List Ports from the Command Line

Prefer a command instead of clicking around? Run this once with the board unplugged and once plugged in. The port that newly appears is your ESP32.
